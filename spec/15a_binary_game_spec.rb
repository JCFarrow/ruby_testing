...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('12', 'wrong string', '2')
      end
  ...
  describe '#display_turn_order' do
    ...
    before do
      ...
      allow(game_turn_order).to receive(:display_guess).with(binary_search)
    ...
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(binary_search) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess)
      game_turn_order.display_turn_order(binary_search)
    end
