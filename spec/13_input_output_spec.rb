...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_wrong_guess) { described_class.new(3, '8')}
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_wrong_guess).not_to be_game_over
      end
    end
  end
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        input_values = ['wrong', 'bad', '9']
        allow(game_loop).to receive(:player_input).and_return(*input_values)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_three_guesses) { described_class.new(1, '1', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_three_guesses.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_five_guesses) { described_class.new(7, '7', 5) }

      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        hard_phrase = "That was hard. It took you 5 guesses!\n"
        expect { game_five_guesses.final_message }.to output(hard_phrase).to_stdout
      end
    end
  end
end
