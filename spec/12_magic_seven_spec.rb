...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 53 and previous step is 60' do
      it 'returns 7' do
        game_with_53 = described_class.new(53)
        previous_step = 60
        result = game_with_53.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 32' do
      ...
      it 'will return 7' do
        game_with_32 = described_class.new(32)
        result = game_with_32.play
        expect(result).to eq(7)
      end
    end
    ...
    context 'when the random number is 12' do
      # remove the 'x' before running this test
      it 'will return 7' do
        game_with_12 = described_class.new(12)
        result = game_with_12.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is random' do
      # remove the 'x' before running this test
      it 'will return 7' do
        random_game = described_class.new
        result = random_game.play
        expect(result).to eq(7)
      end
    end
  end
