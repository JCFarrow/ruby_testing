...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game5to9) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        expect(game5to9.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game8to9) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        expect(game8to9.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game0to3) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        expect(game0to3.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game3to3) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        expect(game3to3.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are not equal' do
      let(:random_number) { double('random_number', value: 0) }
      subject(:game_not_over) { described_class.new(0, 9, random_number, 9) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min = 5, and max = 8' do
      ...
      subject(:game_edge_case) { described_class.new(5, 8, random_number, 7) }

      it 'updates min to the same value as max' do
        game_edge_case.update_range
        expect(game_edge_case.min).to eq(8)
      end

      it 'does not update max' do
        game_edge_case.update_range
        expect(game_edge_case.max).to eq(8)
      end
    end
  end
end
