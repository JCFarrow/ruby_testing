...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_mango) {described_class.new('mango')}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(my_mango.type).to eq('mango')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(my_mango.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_lama) { described_class.new('lama') }

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_lama).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_syrup) { described_class.new('syrup', 2) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_syrup).not_to be_full
      end
    end
  end
end
